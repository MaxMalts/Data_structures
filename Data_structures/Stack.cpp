...
/*  Не для пользователя
*	Тестирует функцию преобразования elem_t в строку, выводя информацию в консоль
*
*	@param[in] fileName Имя файла с тестовыми данными. Файл имеет следующий фомат:\n
В первой строке - количество тестов N\n
В следующих N строках - int и его строковое представление, т.е. два одинаковых числа
*
*	@return 0 - функция работает некорректно; 1 - функция работает правильно
*/

int elem_tToStrTest(const char* fileName) {
	...
	bool success = 1;
	int NTests = 0;
	fscanf(testFile, "%d", &NTests);

	printf("\n");
	for (int i = 0; i < NTests; i++) {
		elem_t testVal = 0;
		char ValStr[20] = {};
		fscanf(testFile, "%d%s", &testVal, ValStr);
		char* retStr = elem_tToStr(testVal);
		if (strcmp(ValStr, retStr) != 0) {
			printf("elme_tToStrTest:   test %d:   failed\n", i);
			success = 0;
		}
		else {
			printf("elme_tToStrTest:   test %d:   succeed\n", i);
		}
	}
...
*	@param[in] stk Стэк
*
*	@return 0 - стэк некорректный; 1 - стэк корректный
*/

#ifdef _DEBUG
int StkOk(stack_t* stk) {
